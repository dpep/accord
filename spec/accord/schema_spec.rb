...
describe Accord::Schema do
  ...
  describe "parsing" do
    ...
    it "raises on non-hash root input (a caller error, not per-field required)" do
      expect { schema.parse("garbage") }.to raise_error(ArgumentError, /Hash-like/)
      expect { schema.parse([1, 2]) }.to raise_error(ArgumentError)
    end
  end
