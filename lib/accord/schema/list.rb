...
module Accord
  class Schema
    ...
    # A plain Schema is always an object (named properties); a List is a
    # top-level JSON array of one, so it lives here as a special case of Schema
    # rather than a separate top-level type. Parsing runs each element through the
    # element schema at its index (errors read `[2, :salary]`, no wrapper key),
    # and every projection is array-shaped — delegated to an ArrayField over the
    # element so OpenAPI/RBS/RBI/GraphQL match the field form exactly.
    #
    #   people = Accord::Schema::List.new(Employee)
